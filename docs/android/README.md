...
## 無線デバッグが切れた時（2026-09-12 追記）

端末の無線デバッグは、端末の再起動などで OFF に戻る。2026-09-10 11:18 に実際に切れ、
見張り役は動いたまま 2 日間 端末を掴めなかった（adb は古い口へ繋ぎ直そうとするだけで、新しい口を探さない）。

- 直し方: 端末で 設定 → 開発者オプション → ワイヤレス デバッグ を ON にするだけ
  （この Wi-Fi は「常に許可」済み。main03 は「ペア設定済みのデバイス」に居る）
