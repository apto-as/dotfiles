...
### 操作 13 の追記（2026-09-15 21:2x）端末が戻ったので、9/14 12:37 の原因を分けた
```
9/14 12:37:36  親機側から切断（local_gen=false・5620MHz・電波 -64）→ 1 秒後に見張り役が掴めなくなる
その後 9/15 19:55 まで、どの網にも新しく繋いでいない（ConnectionEvent は 9/02 から 25 件残っていて、この間は 0 件）
9/15 19:55     自動再接続を OFF にした網の 1 つに繋いだ（自動再接続は OFF のままなので、手で繋いだと読める）→ 20:13 切断
9/15 20:41     家の網に繋いだ
9/15 20:42:08  無線デバッグ ON（端末の記録 adbwifi started）→ 20:49:06 見張り役が繋ぎ直した（探す間隔が 8 分まで延びていた）
9/15 21:04・21:21  接続 2 回とも、ロック解除 → 共有 → LINE → 切断 → ロック まで自動で通った
9/15 21:04:23  AnyDesk の操作権限（AD1）が落ちていたのを、見張り役が戻した（9/07 に続き 2 回目・原因未特定）
端末の稼働 12 日 22 時間（再起動なし）。9/13 に変えた 3 つの設定はそのまま（21:26 に確認）
```
見立て: 家に居れば家の網（自動再接続 ON）へ数分で繋ぎ直すはずなので、12:37 に端末ごと家を出た可能性が高い。出た時刻の記録は無く、確かめてはいない。
言えること: 9/13 の手当て（操作 12）が効かなかった証拠は無い。今回の切断は手当ての範囲外（親機側の切断、または持ち出し）。
操作 14 の実地: 「家の網に見えない」は本物の端末で出た（19:13:54）。「家の網に居るが、無線の口が無い」は出ていない。家の網に入った 20:41:26 から ON の 20:42:08 までの 42 秒に、見張り役の探す番が来なかったため。
