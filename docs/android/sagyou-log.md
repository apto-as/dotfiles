...
## 操作 14: 見張り役に「網に居ない」と「網に居るが無線デバッグが OFF」を分けて記録させた（2026-09-15 19:1x）
### なぜ
9/14 12:37 に切れた時、記録の「無線の口が見つからない」だけでは、持ち出し中か、家に居て無線デバッグが OFF かを言えなかった（操作 13）。
### 変えたもの（anydesk-gate.sh・Mac 側だけ・端末は触っていない）
```
口が見つからない時に、台帳のアドレスへ ping を 3 回 打つ（1 回でも応答すれば「居る」）
状態が変わった時だけ 1 行: 「端末は家の網に居るが、無線の口が無い」／「端末が家の網に見えない」／「台帳にアドレスが無い」
前の「無線の口が見つからない」の 1 行は、上の 3 つに置き換えた
試験用の入口 --test-lan を足した（ANDROID_LAN_ADDR で判定の相手を差し替えられる）
```
### 戻し方
```
git -C ~/dotfiles revert <この変更の commit>
pkill -f 'bash .*anydesk-gate.sh'; nohup ~/dotfiles/scripts/android/anydesk-gate.sh >/dev/null 2>&1 &
```
### 直後の実測（19:12〜19:15）
```
bash -n                          書き方の誤りなし
--test-lan 台帳の端末              見えない（いま家に居ない）
--test-lan 親機（陽性対照）         居る
--test-lan 文書用に予約された住所    見えない
--test-lan 台帳が無い              アドレス無し
見張り役を起こし直した 19:13:22 → 19:13:54「端末が家の網に見えない」を記録
試験用の 2 本目（判定の相手だけ親機・別の記録 file・解除の図形なし）→ 19:15:05「端末は家の網に居るが、無線の口が無い」を記録して止めた
本物の見張り役は 1 本だけ残っている（親の番号は 1 ＝ この作業を終えても止まらない）
```
### 残るもの
```
寝ている端末が ping に応答しない場合、家に居ても「見えない」と出うる。本物の端末で「居る」側が出たのはまだ見ていない
```
